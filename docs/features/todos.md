...
Users can also import visible active todos into the planner. The import action appends todos due today first, followed by todos without a due date. Imported planner rows start with an empty start time and use the todo title as the planner title. Import skips todos whose trimmed title already exists in the planner and skips later todos with the same trimmed title in the same import batch.
